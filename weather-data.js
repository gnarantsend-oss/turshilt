...
window.wIcon = (code) => {
  if (code < 300) return '⛈️'; if (code < 600) return '🌧️';
  if (code < 700) return '❄️'; if (code < 800) return '🌫️';
  if (code === 800) return '☀️'; if (code <= 802) return '⛅';
  return '☁️';
};

window.wColor = (t) => {
  if (t < -25) return '#4fc3f7'; if (t < -10) return '#81d4fa';
  if (t <   0) return '#b3e5fc'; if (t <  15) return '#fff59d';
  if (t <  25) return '#ffcc80'; return '#ef9a9a';
};
