...
const TRENDING    = byTag("trending").slice(0, 12);
const TOP10       = byTag("top10").slice(0, 10);
const NEW_MOVIES  = byTag("new").slice(0, 12);
const HORROR      = byGenre("Аймшиг").slice(0, 12);
const ACTION      = byGenre("Экшн").slice(0, 12);
const POPULAR     = byTag("popular").slice(0, 12);

function MovieCard({ movie, wide = false }: { movie: Movie; wide?: boolean }) {
  const imgSrc = movie.poster || movie.banner;
  return (
    <div className={`movie-card ${wide ? "min-w-[280px] md:min-w-[320px] h-[175px]" : "min-w-[130px] md:min-w-[160px] h-[190px] md:h-[230px]"}`}>
      {imgSrc ? (
        <Image src={imgSrc} alt={movie.title} fill className="object-cover" sizes="170px" />
      ) : (
        <div className="w-full h-full bg-[#2a2a2a] flex items-center justify-center p-2">
          <span className="text-gray-500 text-xs text-center">{movie.title}</span>
        </div>
      )}
      <div className="card-overlay">
        {movie.genre[0] && <span className="text-[10px] text-gray-400 mb-1">{movie.genre[0].name}</span>}
        ...
        <div className="flex gap-2 mt-2">
          <a href={movie.bunnyEmbedUrl} className="w-6 h-6 bg-white rounded-full flex items-center justify-center hover:bg-white/80 transition">
            <Play size={10} fill="black" className="text-black" />
          </a>
          <button className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center hover:bg-white/30 transition border border-white/40"><Plus size={10} /></button>
          <button className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center hover:bg-white/30 transition border border-white/40"><ThumbsUp size={10} /></button>
          <button className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center hover:bg-white/30 transition border border-white/40 ml-auto"><ChevronDown size={10} /></button>
        </div>
      </div>
    </div>
  );
...
function MovieRow({ title, movies: list, wide = false }: { title: string; movies: Movie[]; wide?: boolean }) {
  ...
  return (
    <section className="row-section px-6 md:px-14 mb-8">
      ...
      <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide">
        {list.map((m) => <MovieCard key={m.id} movie={m} wide={wide} />)}
      </div>
    </section>
  );
}
      ...
      <div className="flex gap-1 overflow-x-auto pb-3 scrollbar-hide">
        {list.map((m, i) => (
          <div key={m.id} className="relative flex items-end flex-shrink-0">
            <span className="top10-number select-none" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>{i + 1}</span>
            <div className="ml-8">
              <div className="movie-card min-w-[105px] h-[145px]">
                {m.poster ? (
                  <Image src={m.poster} alt={m.title} fill className="object-cover" sizes="110px" />
                ) : (
                  <div className="w-full h-full bg-[#2a2a2a] flex items-center justify-center p-2">
  ...
  return (
    <main className="min-h-screen bg-[#141414] text-white">
      ...
      <div className="relative h-[90vh] w-full overflow-hidden">
        {heroBanner && (
          <Image src={heroBanner} alt={HERO.title} fill priority className="absolute inset-0 object-cover" sizes="100vw" />
        )}
        ...
        <div className="absolute bottom-[22%] left-6 md:left-14 max-w-xl z-10">
          <div className="badge hero-title"><span>⬛</span> NABO Онцлох</div>
          <h1
          ...
          <div className="hero-desc flex flex-wrap items-center gap-2 text-sm mb-3 text-gray-300">
            ...
            {HERO.genre.map((g) => <span key={g.id} className="genre-dot">{g.name}</span>)}
          </div>
          {HERO.overview && (
      ...
      <div className="-mt-20 relative z-20 pb-20">
        ...
        <Top10Row list={TOP10} />
        <MovieRow title="🆕 Шинэ нэмэгдсэн" movies={NEW_MOVIES} wide />
